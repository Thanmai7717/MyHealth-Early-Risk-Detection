...
def load_research_data():
    ...
    files = {
        'p': os.path.join(base_dir, "patients.csv"),
        'o': os.path.join(base_dir, "observations.csv"),
        'c': os.path.join(base_dir, "conditions.csv")
    }
    ...
    for name, path in files.items():
        if not os.path.exists(path):
            ...
            st.info("Check if the folder name is correct or if the file is missing.")
            st.stop()
            
    p = pd.read_csv(files['p'])
    o = pd.read_csv(files['o'])
    c = pd.read_csv(files['c'])
...
def train_rf_model(df_p, df_o, df_c):
    """
    Trains the Random Forest model using parameters from model.ipynb.
    Target: Chronic Kidney Disease detection.
    """
    # Define clinical markers for training
    ...
    # Merge clinical data with demographics
    df_ml = pd.merge(v_pivot, df_p[['Id', 'AGE', 'GENDER_NUM', 'INCOME']], left_on='PATIENT', right_on='Id')
    
    # Create Target Label (1 if patient has Kidney Disease, 0 otherwise)
    ...
    # Random Forest Setup (79% Accuracy settings)
...
try:
    ...
    tab1, tab2, tab3 = st.tabs(["📊 Patient Dashboard", "🧠 Risk Prediction Engine", "📂 Records"])

    # Helper: Get latest clinical value
    ...
    with tab1:
        ...
        c4.metric("Creatinine", f"{get_v('Creatinine')} mg/dL")
    ...
    with tab2:
        ...
        # Mapping input data to match model training
        inputs = {
            'AGE': patient['AGE'],
        ...
        with res_col:
            ...
            if risk_pct > 70:
                st.error("HIGH RISK: Markers indicate potential renal decline.")
            elif risk_pct > 30:
                st.warning("ELEVATED: Early metabolic monitoring recommended.")
            else:
                st.success("STABLE: Risk factors within normal limits.")
        with bar_col:
            st.progress(risk_score)
            st.caption("AI probability based on longitudinal clinical data.")

    # TAB 3: RECORDS
    with tab3:
        st.subheader("Clinical Data Export")
        uploaded_json = st.file_uploader("Upload Hospital Visit Summary (JSON)", type=['json'])
        if uploaded_json:
