...
def load_data():
    ...
    p = load_file('patients.csv')
    o = load_file('observations.csv')
    c = load_file('conditions.csv')
    e = load_file('encounters.csv')
    return p, o, c, e
...
def train_models(model_data):
    ...
    return (lasso_model, scaler, lasso_acc, lasso_report,
            xgb_model, xgb_acc, xgb_report, X, X_train, X_test, y_test)
...
try:
    ...
    with st.spinner("Training models... please wait"):
        ...
        (lasso_model, scaler, lasso_acc, lasso_report,
         ...
         X, X_train, X_test, y_test) = train_models(model_data)
    ...
    # ── TAB: MODEL INSIGHTS ───────────────────────────────────
        ...
        with r1:
            st.markdown("**XGBoost**")
            xgb_df = pd.DataFrame(xgb_report).transpose().round(2)
            st.dataframe(xgb_df)

        with r2:
            st.markdown("**Lasso**")
            lasso_df = pd.DataFrame(lasso_report).transpose().round(2)
